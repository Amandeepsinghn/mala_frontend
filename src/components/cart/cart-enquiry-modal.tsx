...
import type { PreferredContact } from "@/types/inquiry";
...
function CartEnquiryForm({
  ...
  function buildCartSummary(): string {
    return items
      .map(
        (item) =>
          `- ${item.product.name} (qty: ${item.quantity}, ${formatPrice(item.product.price * item.quantity, item.product.currency)})`,
      )
      .join("\n");
  }
    ...
    return (
      <div className="py-8 text-center">
        ...
        <p className="mt-4 text-stone-600">
          We received your cart enquiry. Our team will reach out via your
  ...
  return (
    <form id={formId} onSubmit={handleSubmit}>
      ...
      <ul className="mt-4 space-y-2 rounded-md border border-stone-200 bg-stone-50 px-4 py-3 text-sm text-stone-700">
        {items.map((item) => (
          <li key={item.cartItemId ?? item.product.id}>
    ...
    >
      <div className="mx-auto min-h-full max-w-3xl px-4 py-10 sm:px-6 sm:py-16 lg:px-8">
        ...
        <div className="rounded-lg bg-white px-6 py-8 sm:px-10 sm:py-12">
          <CartEnquiryForm items={items} onClose={onClose} />
        </div>
      </div>
    </div>
  );
